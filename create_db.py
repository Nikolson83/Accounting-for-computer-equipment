...
class DB:
    ...
    def insert_data(self, add_name, add_inv_nomer, add_pib, add_division, add_data_in, add_data_out):
        self.c.execute(
            '''INSERT INTO accounting(add_name, add_inv_nomer, add_pib, add_division, add_data_in, add_data_out)VALUES (?, ?, ?, ?, ?, ?)''',
    ...
    def update_records(self, add_name, add_inv_nomer, add_pib, add_division, add_data_in, add_data_out, row_id):
        self.c.execute(
            '''UPDATE accounting SET add_name=?, add_inv_nomer=?, add_pib=?, add_division=?, add_data_in=?, add_data_out=? WHERE add_id=?''',
            (add_name, add_inv_nomer, add_pib, add_division, add_data_in, add_data_out,
             row_id))  # Обновление данных и вытягивает ID с первого столбца
